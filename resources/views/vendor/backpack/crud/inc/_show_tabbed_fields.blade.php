...
<div class="tab-content {{$horizontalTabs ? 'col-md-12' : 'col-md-9 m-t-10'}}">
    ...
    <div role="tabpanel" class="tab-pane{{$k == 0 ? ' active' : ''}}" id="tab_{{ str_slug($tab, "") }}">
        ...
        <?php
        $fields_for_current_tab = collect($fields)->filter(function ($value, $key) use ($tab) {
            return isset($value['tab']) && $value['tab'] == $tab;
        });
        ?>
        @include('crud::inc.show_fields', ['fields' => $fields_for_current_tab])
