            ...
            $("#select2_ajax_{{ $f_id.$r_id }}").each(function (i, obj) {
                if (!$(obj).hasClass("select2-hidden-accessible")) {
                    $(obj).select2({
                            ...
                            processResults: function (data, params) {
                                ...
                                var result = {
                                    results: $.map(data.data, function (item) {
                                        ...
                                        return {
                                            text: item["account_number"] + ' - ' + item["customer_name_kh"] + ' - ' + item["customer_name"],
                                            id: item["id"]
                                        }
                                    }),
                        ...
                        var q_customer_id = $(this).val();

                        $.ajax({
                            method: "GET",
                            url: "{{ $field['data_source'] }}" + "/" + q_customer_id,
                            dataType: "json",
                            success: function (obj) {
                                $('[name="customer_account_id"]').val(obj.id);
                                $.each(obj, function (name, value) {
                                    //console.log(name + ": " + value);
                                    if (name && value) {
                                        if (name == 'province_op' || name == 'district_op' || name == 'commune_op' ||  name == 'village_op' ) {
                                            $('[name="' + name.replace('_op','_id') + '"]').html(value);
                                        } else if(name == 'province_id' ||  name == 'district_id' ||  name == 'commune_id' || name == 'village_id'){
