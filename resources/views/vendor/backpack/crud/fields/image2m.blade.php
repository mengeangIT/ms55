@php
$f = isset($field['value'])?json_decode($field['value'],true) : [];
@endphp
...
@if(count($f)>0)
    @foreach($f as $fv)
        ...
        @php
            $r_id = rand(1,9999);
            $f_id = isset($field['name'])? str_replace(']','',str_replace('[','',$field['name'])) :'my-id';
        @endphp
        <div class="form-group col-md-12 image" data-preview="#{{ $f_id.$r_id }}" data-aspectRatio="{{ isset($field['aspect_ratio']) ? $field['aspect_ratio'] : 0 }}" data-crop="{{ isset($field['crop']) ? $field['crop'] : false }}" @include('crud::inc.field_wrapper_attributes')>
            ...
            <div class="row">
                <div class="col-sm-6" style="margin-bottom: 20px;">
                    <img id="mainImage" src="{{ url( (isset($field['prefix']) ? $field['prefix'] : '') . (old($field['name']) ? old($field['name']) : (isset($fv) ? $fv : (isset($field['default']) ? $field['default'] : '') ))) }}">
                </div>
                @if(isset($field['crop']) && $field['crop'])
                    <div class="col-sm-3">
                        <div class="docs-preview clearfix">
                            <div id="{{ $f_id.$r_id }}" class="img-preview preview-lg">
            ...
            <div class="btn-group">
                ...
                <button class="btn btn-success add-m" type="button"><i class="fa fa-plus"></i></button>
            </div>
...
@if ($crud->checkIfFieldIsFirstOfItsType($field, $fields))
    ...
    <script>
        jQuery(document).ready(function($) {
            ...
            $('body').delegate('.add-m','click',function(){
                
                var m_id = Math.floor(Math.random() * 26) + Date.now();

                var p = $(this).parent().parent();
                var html = '<div class="form-group col-md-12 image" data-preview="#image'+m_id+'" data-aspectratio="1.5" data-crop="1">\n' +
                    '    <div>\n' +
                    '        <label>Image</label>\n' +
                    '            </div>\n' +
                    '    <!-- Wrap the image or canvas element with a block element (container) -->\n' +
                    '    <div class="row">\n' +
                    '        <div class="col-sm-6" style="margin-bottom: 20px;">\n' +
                    '            <img id="mainImage" src="http://localhost:8000/No_Image_Available.jpg">\n' +
                    '        </div>\n' +
                    '                    <div class="col-sm-3">\n' +
                    '                <div class="docs-preview clearfix">\n' +
                    '                    <div id="image'+m_id+'" class="img-preview preview-lg">\n' +
                    '                        <img src="" style="display: block; min-width: 0px !important; min-height: 0px !important; max-width: none !important; max-height: none !important; margin-left: -32.875px; margin-top: -18.4922px; transform: none;">\n' +
                    '                    </div>\n' +
                    '                </div>\n' +
                    '            </div>\n' +
                    ...
                    '            <input type="hidden" id="hiddenImage" name="{{ $field['name'] }}[n]['+m_id+']" value="http://localhost:8000/No_Image_Available.jpg">\n' +
                    '        </label>\n' +
                    '                    <button class="btn btn-default" id="rotateLeft" type="button" style="display: none;"><i class="fa fa-rotate-left"></i></button>\n' +
                    '            <button class="btn btn-default" id="rotateRight" type="button" style="display: none;"><i class="fa fa-rotate-right"></i></button>\n' +
                    '            <button class="btn btn-default" id="zoomIn" type="button" style="display: none;"><i class="fa fa-search-plus"></i></button>\n' +
